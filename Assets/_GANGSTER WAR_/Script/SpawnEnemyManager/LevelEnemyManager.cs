    ...
    IEnumerator SpawnEnemyCo()
    {
        for (int i = 0; i < EnemyWaves.Length; i++)
        {
            ...
            for (int j = 0; j < EnemyWaves[i].enemySpawns.Length; j++)
            {
                ...
                for (int k = 0; k < enemySpawn.numberEnemy; k++)
                {
                    GameObject _temp = Instantiate(enemySpawn.enemy, new Vector2(Random.Range(spawnPositionA.position.x, spawnPositionB.position.x),
                        Random.Range(spawnPositionA.position.y, spawnPositionB.position.y)), Quaternion.identity) as GameObject;
